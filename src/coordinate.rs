...
impl CoordinateTransformer {
    ...
    pub fn is_at_transfer_edge(&self, local: &LocalCoordinate) -> bool {
        ...
        match self.config.edge.sender_to_receiver {
            ...
            EdgeDirection::Left => {
                // 自分が右側の場合、左端で転送
                matches!(self.config.layout.position, Position::Right) && 
                local.x <= 5.0
            },
            ...
            EdgeDirection::Top => {
                // 自分が下側の場合、上端で転送
                matches!(self.config.layout.position, Position::Bottom) && 
                local.y <= 5.0
            },
        }
    }
    ...
    pub fn get_virtual_screen_size(&self) -> (u32, u32) {
        match self.config.layout.position {
            Position::Left | Position::Right => {
                ...
                let max_height = self.config.screen.height.max(self.config.remote_screen.height);
                (total_width, max_height)
            },
            Position::Top | Position::Bottom => {
                let max_width = self.config.screen.width.max(self.config.remote_screen.width);
